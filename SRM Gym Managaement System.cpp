...
int alogin::getadpassword()
{
	...
	cin.sync(); //to clear buffer memory
	getline(cin, pw);//string input
	if (pw == "SMHVS")
	{
		return 1;
	}
...
public:
	...
	void create_mem()
	{
		...
		getchar();
		cin.getline(mem_name, 50);//string input
		cout << endl
...
void save_member()
{
	fp.open("Gymdetails.txt", ios::out | ios::app);//opening file and out for writing on file and app for appending 
	m1.create_mem();
	fp.write((char *)&m1, sizeof(m1)); // & for passing reference, sizeof for no. of bytes req.
	fp.close();
	cout << endl
		 << endl
...
void show_all()
{
	...
	cout << endl
		 << "\t\t-- ALL RECORDS --";
	bool found = false;
	fp.open("Gymdetails.txt", ios::in);//in for reading
...
void edit_member()
{
	...
	fp.open("Gymdetails.txt", ios::in | ios::out);
...
void delete_member()
{
	...
	fp2.close();
	fp.close();
	remove("Gymdetails.txt");
	rename("Temp2.txt", "Gymdetails.txt");
	cout << endl
		 << endl
		 << "\tRecord Deleted...";
	cin.sync();
